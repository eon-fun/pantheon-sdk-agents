...
from pantheon_sdk.agents.p2p.config import P2PConfig
from pantheon_sdk.agents.p2p.libp2p.node import LibP2PNode
...
@pytest.fixture()
def node(config):
    with patch("pantheon_sdk.agents.p2p.libp2p.node.load_or_create_node_key") as mock_load_key:
...
class TestLibP2PNode:
    ...
    def test_init_host(self, node):
        with (
            patch("pantheon_sdk.agents.p2p.libp2p.node.new_host") as mock_new_host,
            patch("pantheon_sdk.agents.p2p.libp2p.node.decode_noise_key") as mock_decode,
            patch("pantheon_sdk.agents.p2p.libp2p.node.NoiseTransport") as mock_noise,
        ):
    ...
    async def test_initialize(self, node):
        ...
        with (
            ...
            patch("pantheon_sdk.agents.p2p.libp2p.node.CircuitV2Protocol"),
            patch("pantheon_sdk.agents.p2p.libp2p.node.CircuitV2Transport") as mock_transport,
        ):
    ...
    async def test_connect_to_relay_success(self, node):
        ...
        with patch("pantheon_sdk.agents.p2p.libp2p.node.info_from_p2p_addr") as mock_info:
    ...
    async def test_connect_to_relay_failure(self, node):
        ...
        with patch("pantheon_sdk.agents.p2p.libp2p.node.info_from_p2p_addr"):
            result = await node.connect_to_relay()
    ...
    async def test_setup_listener(self, node):
        ...
        with patch("pantheon_sdk.agents.p2p.libp2p.node.handle_card"):
            await node.setup_listener(mock_nursery)
            assert mock_transport.create_listener.call_count == 1

        # Second call (handle_handoff)
        with patch("pantheon_sdk.agents.p2p.libp2p.node.handle_handoff"):
            await node.setup_listener(mock_nursery)
